...
function createExecutor(driver, options) {
    ...
    if (options && options.services && options.services.length) {
        serviceClasses = options.services.filter(function (service) { return service instanceof Function; });
        var serviceDirs = options.services.filter(function (service) { return typeof service === "string"; });
        serviceClasses.push.apply(serviceClasses, importClassesFromDirectories_1.importClassesFromDirectories(serviceDirs));
    }
